...
const STREAMS: usize = 7; // FM1..FM6 + DAC
...
fn make_chip(clock: u32, stream: usize) -> Result<StreamChip, String> {
    ...
    for ch in 0..6u32 {
        ...
        chip.pin_mut().write(port, reg - if ch >= 3 { 3 } else { 0 });
...
fn render_wait(streams: &mut [StreamChip], channels: usize, native_rate: u32, wait: u64,
              timing: &mut TimingAccumulator, skip_frames: &mut u64, peak: &mut i32) -> u64 {
    if wait == 0 { return 0; }
    let n = timing.frames_for_wait(wait, native_rate);
    if n == 0 { return 0; }
    // The same skip amount must be applied to EVERY stream. Applying it
    // separately while mutating skip_frames would make FM1 full-length, FM2
    // shorter, FM3 shorter again, etc., which sounds like channels playing
    // in sequence and also breaks timing.
...
fn run() -> Result<(), String> {
    ...
    println!("channel_streams=FM1,FM2,FM3,FM4,FM5,FM6,DAC");
    ...
    let mut streams=Vec::with_capacity(STREAMS); for s in 0..STREAMS { streams.push(make_chip(clock,s)?); }
    ...
    while !ended {
        ...
        match cmd {
            ...
            0x61=>{let n=u16::from_le_bytes([bytes[pos+1],bytes[pos+2]]) as u64;render_wait(&mut streams,channels,rate,n,&mut timing,&mut skip_frames,&mut peak);waits+=n;timeline_samples+=n;pos+=3;}
            0x62=>{render_wait(&mut streams,channels,rate,735,&mut timing,&mut skip_frames,&mut peak);waits+=735;timeline_samples+=735;pos+=1;}
            0x63=>{render_wait(&mut streams,channels,rate,882,&mut timing,&mut skip_frames,&mut peak);waits+=882;timeline_samples+=882;pos+=1;}
            ...
            0x70..=0x7f=>{let n=(cmd&0x0f) as u64+1;render_wait(&mut streams,channels,rate,n,&mut timing,&mut skip_frames,&mut peak);waits+=n;timeline_samples+=n;pos+=1;}
            0x80..=0x8f=>{let bank=blocks.get(&0).ok_or("DAC bank missing")?;if dac_pos>=bank.len(){return Err("DAC bank exhausted".into());}let v=bank[dac_pos];dac_pos+=1;for (s,sc) in streams.iter_mut().enumerate(){apply_ym_write(sc,s,0x52,0x2a,v);}writes+=1;dac_writes+=1;let n=(cmd&0x0f) as u64;render_wait(&mut streams,channels,rate,n,&mut timing,&mut skip_frames,&mut peak);waits+=n;timeline_samples+=n;pos+=1;}
